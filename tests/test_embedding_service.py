...
class TestEmbeddingService:
    ...
    def embedding_service(self, mock_cache, mock_openai_client):
        with patch('services.preprocessing.embedding_service.RedisEmbeddingCache', return_value=mock_cache), \
             patch('services.preprocessing.embedding_service.AsyncOpenAI', return_value=mock_openai_client):
    ...
    @pytest.mark.asyncio
    async def test_get_embedding_cache_miss_success(self, embedding_service, mock_cache, mock_openai_client):
        test_text = "test text"
        expected_embedding = [0.1, 0.2, 0.3]
    ...
    @pytest.mark.asyncio
    async def test_get_embedding_api_error_fallback(self, embedding_service, mock_cache, mock_openai_client):
        test_text = "test text"
        mock_cache.get.return_value = None
    ...
    @pytest.mark.asyncio
    async def test_get_embedding_empty_text(self, embedding_service, mock_cache, mock_openai_client):
        test_text = ""
    ...
    def test_service_initialization(self, mock_cache, mock_openai_client):
        with patch('services.preprocessing.embedding_service.RedisEmbeddingCache', return_value=mock_cache), \
             patch('services.preprocessing.embedding_service.AsyncOpenAI', return_value=mock_openai_client), \
             patch('services.preprocessing.embedding_service.settings.OPENAI_API_KEY', 'test-key'), \
             patch('services.preprocessing.embedding_service.settings.EMBEDDING_BATCH_DELAY', 1.5):
