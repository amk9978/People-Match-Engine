...
class TestFastEmbeddingService:
    ...
    def fast_embedding_service(self, mock_cache, mock_text_embedding_model):
        with patch('services.preprocessing.fast_embedding_service.RedisEmbeddingCache', return_value=mock_cache), \
             patch('services.preprocessing.fast_embedding_service.TextEmbedding', return_value=mock_text_embedding_model):
    ...
    @pytest.mark.asyncio
    async def test_get_embedding_cache_miss_success(self, fast_embedding_service, mock_cache, mock_text_embedding_model):
        test_text = "test text"
    ...
    @pytest.mark.asyncio
    async def test_get_embedding_model_error_fallback(self, fast_embedding_service, mock_cache, mock_text_embedding_model):
        test_text = "test text"
        mock_cache.get.return_value = None
    ...
    @pytest.mark.asyncio
    async def test_get_batch_embeddings_all_cached(self, fast_embedding_service, mock_cache):
    ...
    @pytest.mark.asyncio
    async def test_get_batch_embeddings_mixed_cache(self, fast_embedding_service, mock_cache, mock_text_embedding_model):
        ...
        expected_result = [cached_embedding, uncached_embeddings[0], uncached_embeddings[1]]
        assert result == expected_result
        assert mock_cache.get.call_count == 3
        mock_text_embedding_model.embed.assert_called_once_with(["uncached_text1", "uncached_text2"])
        assert mock_cache.set.call_count == 2
    
    @pytest.mark.asyncio
    async def test_get_batch_embeddings_validation_error(self, fast_embedding_service, mock_cache, mock_text_embedding_model):
        texts = ["text1", "text2"]
        mock_cache.get.return_value = None
    ...
    @pytest.mark.asyncio
    async def test_get_batch_embeddings_batch_error_fallback(self, fast_embedding_service, mock_cache, mock_text_embedding_model):
        texts = ["text1", "text2"]
        mock_cache.get.return_value = None
    ...
    def test_service_initialization(self, mock_cache, mock_text_embedding_model):
        with patch('services.preprocessing.fast_embedding_service.RedisEmbeddingCache', return_value=mock_cache), \
             patch('services.preprocessing.fast_embedding_service.TextEmbedding', return_value=mock_text_embedding_model), \
             patch('services.preprocessing.fast_embedding_service.settings.EMBEDDING_BATCH_DELAY', 2.5):
