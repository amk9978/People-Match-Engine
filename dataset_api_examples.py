...
def example_1_list_datasets():
    ...
    if response.status_code == 200:
        ...
        for dataset in data["datasets"]:
            print(
                f"  • {dataset['filename']} ({dataset['current_rows']} rows, {dataset['total_versions']} versions)"
            )
    else:
...
def example_2_get_dataset_info(filename):
    ...
    if response.status_code == 200:
        ...
        for v in info["versions"][-3:]:  # Show last 3 versions
            changes = v["changes"]
            print(
                f"    - {v['version_id'][:8]}: {v['type']} (+{changes['added_rows']}/-{changes['deleted_rows']} rows) - {v['description']}"
            )
    else:
        print(f"❌ Error: {response.text}")
...
def example_3_preview_dataset(filename):
    ...
    response = requests.get(
        f"{BASE_URL}/datasets/{filename}/preview?limit=3", headers=HEADERS
    )

    if response.status_code == 200:
        preview = response.json()
        print(
            f"✅ Preview ({preview['preview_rows']} of {preview['total_rows']} rows):"
        )
...
def example_5_delete_by_criteria(filename):
    ...
    delete_criteria = {
        "criteria": {"Professional Identity - Experience Level": "Junior"},
...
def example_8_analyze_modified(filename, version_id=None):
    ...
    response = requests.post(
        f"{BASE_URL}/analyze-modified/{filename}", headers=HEADERS, params=params
    )
