...
class GraphScorer:
    ...
    def _load_default_weights_from_env(self) -> list[float]:
        ...
        try:
            ...
            used_weight = (
                person_similarity + person_complementarity + business_complementarity
            )
            business_similarity = max(0.0, 1.0 - used_weight)

            weights = [
                person_similarity,
                person_complementarity,
                business_similarity,
            ...
            logger.info(
                f"📋 Loaded default weights from .env: {[f'{w:.3f}' for w in weights]}"
            )
            return weights

        except Exception as e:
            logger.info(
                f"⚠️ Error loading weights from .env: {e}, using balanced fallback"
            )
    ...
    async def get_tuned_2plus2_weights(self, user_prompt: str = None) -> List[float]:
        ...
        if not user_prompt or not user_prompt.strip():
            logger.info("🔧 No user prompt provided, using .env defaults")
...
RESPOND ONLY: [number, number, number, number]"""
        ...
        for attempt in range(3):
            try:
                ...
                logger.info(f"🤖 ChatGPT attempt {attempt+1}: {response_text}")
                ...
                if weights and len(weights) == 4:
                    ...
                    if total > 0:
                        ...
                        if all(w >= 0 for w in weights):
                            logger.info(
                                f"🎯 ChatGPT tuned weights for '{user_prompt}': {[f'{w:.3f}' for w in weights]}"
                            )
                            return weights
                ...
                logger.info(f"⚠️ ChatGPT attempt {attempt+1} failed: {e}")
                if attempt == 2:
                    break

        logger.info(f"⚠️ All ChatGPT attempts failed. Using .env default weights")
        return self.default_weights
