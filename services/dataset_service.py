...
class DatasetService:
    ...
    def store_original_dataset(self, user_id: str, filename: str, df: pd.DataFrame) -> Dict:
    ...
    def add_rows(self, user_id: str, filename: str, new_rows: List[Dict], description: str = None) -> Dict:
        ...
        return self._create_new_version(
            user_id, filename, updated_df, "add_rows", 
            {"added_rows": len(new_rows), "deleted_rows": 0},
            description or f"Added {len(new_rows)} rows"
        )

    def delete_rows(self, user_id: str, filename: str, row_indices: List[int], description: str = None) -> Dict:
        ...
        return self._create_new_version(
            user_id, filename, updated_df, "delete_rows",
            {"added_rows": 0, "deleted_rows": len(row_indices)},
            description or f"Deleted {len(row_indices)} rows"
        )

    def delete_rows_by_criteria(self, user_id: str, filename: str, criteria: Dict, description: str = None) -> Dict:
        ...
        for column, value in criteria.items():
            ...
            if isinstance(value, dict):
                ...
                if operator == "equals":
                    ...
                    mask = mask & current_df[column].astype(str).str.contains(str(search_value), na=False)
                elif operator == "not_equals":
                    mask = mask & (current_df[column] != search_value)
                elif operator == "empty":
                    mask = mask & (current_df[column].isna() | (current_df[column].astype(str).str.strip() == ""))
                elif operator == "not_empty":
                    mask = mask & (current_df[column].notna() & (current_df[column].astype(str).str.strip() != ""))
            else:
        ...
        return self._create_new_version(
            user_id, filename, updated_df, "delete_by_criteria",
            {"added_rows": 0, "deleted_rows": deleted_count},
            description or f"Deleted {deleted_count} rows matching criteria"
        )

    def _create_new_version(self, user_id: str, filename: str, df: pd.DataFrame, 
                          operation_type: str, changes: Dict, description: str) -> Dict:
    ...
    def get_dataset_preview(self, user_id: str, filename: str, version_id: str = None, 
    ...
    def revert_to_version(self, user_id: str, filename: str, target_version_id: str, 
        ...
        return self._create_new_version(
            user_id, filename, target_df, "revert",
        ...
        for file_info in user_files:
            ...
            if dataset_info:
                datasets.append({
                    ...
                    "last_modified": dataset_info["versions"][-1]["created_at"] if dataset_info["versions"] else None
                })
