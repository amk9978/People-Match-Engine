...
class FaissOptimizer:
    ...
    def initialize_faiss(self, feature_embeddings: Dict[str, np.ndarray]) -> None:
        ...
        print(f"🚀 Initializing FAISS optimization (top-{self.top_k} per person)...")
    ...
    def get_candidate_pairs(self, min_similarity: float = 0.05) -> Set[Tuple[int, int]]:
        ...
        for feature_name in feature_names:
            try:
                ...
                print(f"   ✓ {feature_name}: {len(similarities)} candidate pairs")
            except Exception as e:
                print(f"   ⚠️ {feature_name}: skipped ({e})")
        ...
        print(f"⚡ FAISS found {len(all_candidate_pairs)} total candidate pairs")
        
        return all_candidate_pairs, feature_similarities

    def get_similarity_scores(self, person_i: int, person_j: int, feature_similarities: Dict) -> Dict[str, float]:
