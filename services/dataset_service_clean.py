...
class DatasetService:
    ...
    def _validate_rows_columns(
        self, rows: List[Dict], expected_columns: List[str]
    ) -> None:
    ...
    def _create_new_version(
        ...
        file_path = self._get_dataset_path(
            dataset.user_id, dataset.filename, version_id
        )
    ...
    def _save_dataframe(
        self, user_id: str, filename: str, version_id: str, df: pd.DataFrame
    ) -> None:
    ...
    def _save_dataset_to_cache(self, dataset: Dataset) -> None:
        ...
        self.cache.set(
            versions_key, json.dumps([v.to_dict() for v in dataset.versions])
        )
