...
class FAISSService:
    ...
    def build_index(
        ...
        if self.embeddings.shape[1] != self.embedding_dim:
            raise ValueError(
                f"Embedding dimension mismatch: expected {self.embedding_dim}, got {self.embeddings.shape[1]}"
            )
...
class FAISSEmbeddingAdapter:
    ...
    def __init__(
        self, embedding_service: EmbeddingServiceProtocol, embedding_dim: int = 384
    ):
    ...
    async def search_similar_texts(
        self, query_text: str, k: int = 5, threshold: Optional[float] = None
    ) -> List[Tuple[int, float, Any]]:
