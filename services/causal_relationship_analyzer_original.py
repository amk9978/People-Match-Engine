...
Respond with ONLY a number between 0.0 and 1.0"""
    ...
    async def build_incremental_persona_matrix(self, new_personas: set) -> None:
        ...
        existing_personas = (
            set(self.persona_matrix.keys()) if self.persona_matrix else set()
        )
        ...
        print(
            f"📈 Adding {len(personas_to_add)} new personas to matrix (total: {len(all_personas)})"
        )
            ...
            for other_persona in all_personas:
                if new_persona != other_persona:
                    ...
                    if other_persona not in self.persona_matrix[new_persona]:
                        try:
                            score = (
                                await self.calculate_persona_complementarity_chatgpt(
                            ...
                            print(
                                f"Error calculating {new_persona} <-> {other_persona}: {e}"
                            )
        ...
        self.cache.set(
            "persona_complementarity_matrix", json.dumps(self.persona_matrix)
        )
        print(
            f"💾 Updated persona matrix cached with {len(self.persona_matrix)} personas"
        )
...
Respond with only a number between 0.0 and 1.0."""
        ...
        try:
            ...
            return await self.calculate_embedding_similarity_fallback(
                persona1, persona2
            )
    ...
    async def build_experience_complementarity_matrix(
        self, csv_path: str
    ) -> Dict[str, Dict[str, float]]:
        ...
        for idx, row in df.iterrows():
            if pd.notna(row.get("Professional Identity - Experience Level", "")):
                tags = tag_extractor.extract_tags(
                    row["Professional Identity - Experience Level"], "experience"
                )
                all_experience_tags.update(tags)

        experience_list = list(all_experience_tags)
        print(
            f"Building experience complementarity matrix for {len(experience_list)} experience levels..."
        )
        ...
        for experience1 in experience_list:
            ...
            for experience2 in experience_list:
                if experience1 == experience2:
                    experience_matrix[experience1][
                        experience2
                    ] = 0.0  # Same experience = no complementarity
                else:
                    # Use ChatGPT to score experience complementarity
                    try:
                        score = await self.calculate_experience_complementarity_chatgpt(
        ...
        # Cache the matrix
        self.cache.set(
            "experience_complementarity_matrix", json.dumps(experience_matrix)
        )
        self.experience_matrix = experience_matrix

        print(
            f"✅ Built experience complementarity matrix with {len(experience_list)} experience levels"
        )
...
Consider only experience levels and their complementarity for professional growth and learning:
- Knowledge transfer potential based on experience gaps
- Mentorship and reverse mentorship opportunities
- Learning exchange based on different career stages
- Professional development synergies

Focus purely on how these experience levels complement each other for mutual professional growth, regardless of their specific roles or industries.
...
Respond with only a number between 0.0 and 1.0."""
    ...
    def extract_role_tags_from_dataset(self, csv_path: str) -> Set[str]:
        """Extract all unique role tags from dataset"""
        df = pd.read_csv(csv_path)

        role_tags = set()

        # Role specification column
        role_column = "Professional Identity - Role Specification"
        if role_column in df.columns:
            for value in df[role_column].dropna():
                # Use tag_extractor for consistency
                from services.tag_extractor import tag_extractor
                tags = tag_extractor.extract_tags(str(value), "role_spec")
                role_tags.update(tags)

        print(f"Extracted {len(role_tags)} unique role tags")
        return role_tags

    async def analyze_role_complementarity(self, role1: str, role2: str) -> float:
        """Analyze complementarity between two role tags using ChatGPT"""

        cache_key = f"role_complementarity:{sorted([role1, role2])}"
        cached_result = self.cache.get(cache_key)
        if cached_result:
            return float(cached_result)

        prompt = f"""You are analyzing professional role complementarity for networking/collaboration purposes.

ROLE 1: {role1}
ROLE 2: {role2}

Rate how well these two professional roles would complement each other for business collaboration/networking on a scale of 0.0 to 1.0:

Consider:
- Do they have complementary skill sets that would benefit collaboration?
- Would they bring different but synergistic perspectives to business problems?
- Are there natural working relationships between these roles?
- Would connecting them create mutual professional value?

Examples:
- "CEO" + "CTO" = 0.9 (perfect executive complementarity)
- "Product Manager" + "Software Engineer" = 0.8 (strong collaboration potential)
- "Sales Director" + "Marketing Director" = 0.7 (business development synergy)
- "Data Scientist" + "Business Analyst" = 0.8 (analytical complementarity)
- "CEO" + "CEO" = 0.3 (similar roles, some peer value but less complementary)
- "Junior Developer" + "Senior Developer" = 0.6 (mentorship value)
- "Founder" + "Investor" = 0.9 (funding/guidance complementarity)

Rate from 0.0 (no complementarity/too similar) to 1.0 (perfect complementarity).
...
Respond with ONLY a number between 0.0 and 1.0"""
        ...
        try:
            ...
            print(f"Error analyzing role complementarity {role1}-{role2}: {e}")
            return 0.5  # Neutral fallback

    async def build_role_complementarity_matrix(
        self, csv_path: str
    ) -> Dict[str, Dict[str, float]]:
        """Build role complementarity adjacency matrix using ChatGPT"""

        # Check if already cached
        cache_key = "role_complementarity_matrix"
        cached_matrix = self.cache.get(cache_key)
        if cached_matrix:
            try:
                self.role_matrix = json.loads(cached_matrix)
        ...
        # Extract all unique role tags
        role_tags = self.extract_role_tags_from_dataset(csv_path)
        role_list = list(role_tags)

        print(f"Analyzing complementarity for {len(role_list)} roles...")

        # Create all pairs for analysis
        all_tasks = []
        for i, role1 in enumerate(role_list):
            for j, role2 in enumerate(role_list):
                if i <= j:  # Only analyze each pair once (symmetric matrix)
                    task = self.analyze_role_complementarity(role1, role2)
                    all_tasks.append(((role1, role2), task))

        print(f"Total role pairs to analyze: {len(all_tasks)}")
        ...
        for result in completed_tasks:
            ...
            (role1, role2), score = result
            if score is not None:
                # Symmetric matrix
                role_matrix[role1][role2] = score
                role_matrix[role2][role1] = score
                successful_count += 1

        print(
            f"Successfully analyzed {successful_count}/{len(all_tasks)} role pairs"
        )

        # Cache the result
        self.cache.set(cache_key, json.dumps(role_matrix))
        self.role_matrix = role_matrix

        print(f"✅ Built role complementarity matrix with {len(role_list)} roles")
        return role_matrix

    def load_role_matrix_from_redis(self) -> bool:
        """Load role complementarity matrix from Redis cache"""
        cached_matrix = self.cache.get("role_complementarity_matrix")
        if cached_matrix:
            try:
                self.role_matrix = json.loads(cached_matrix)
                return True
    ...
    async def calculate_role_complementarity_fast(
        ...
        """Fast calculation of role complementarity using cached matrix"""

        if not hasattr(self, "role_matrix") or not self.role_matrix:
            # Try to load from cache first
            if not self.load_role_matrix_from_redis():
                ...
                for role1 in person1_roles:
                    for role2 in person2_roles:
                        ...
                        total_score += 1.0 - score  # Convert similarity to complementarity
                        pair_count += 1
        ...
        for role1 in person1_roles:
            for role2 in person2_roles:
                if (
                    role1 in self.role_matrix
                    and role2 in self.role_matrix[role1]
                ):
                    total_score += self.role_matrix[role1][role2]
                    pair_count += 1
