...
import pandas as pd
from domain.entities.dataset import Dataset, DatasetModification, DatasetDeletionCriteria
...
class DatasetService:
    ...
    def create_dataset_from_dataframe(self, user_id: str, filename: str, df: pd.DataFrame) -> Dict:
        ...
        if hasattr(self.dataset_repository, 'create_from_dataframe'):
            dataset = self.dataset_repository.create_from_dataframe(user_id, filename, df)
        else:
    ...
    def add_rows(self, user_id: str, filename: str, rows: List[Dict], 
                description: Optional[str] = None) -> Dict:
        """Add new rows to dataset"""
        modification = DatasetModification(rows=rows, description=description)
        new_version = self.modification_service.add_rows(user_id, filename, modification)
    ...
    def delete_rows_by_criteria(self, user_id: str, filename: str, 
                               criteria: Dict, description: Optional[str] = None) -> Dict:
        """Delete rows matching criteria"""
        deletion_criteria = DatasetDeletionCriteria(criteria=criteria, description=description)
    ...
    def load_dataset(self, user_id: str, filename: str, version_id: Optional[str] = None) -> Optional[pd.DataFrame]:
