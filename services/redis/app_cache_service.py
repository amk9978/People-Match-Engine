...
class AppCacheService:
    ...
    def _get_complementarity_cache_key(self, target_profile: str, comparison_profiles: List[str], category: str) -> str:
        ...
        comparison_hash = hashlib.md5(str(sorted(comparison_profiles)).encode()).hexdigest()[:8]
        return f"profile_complementarity_{category}_{target_hash}_vs_{comparison_hash}"

    # Row-based embedding caching
    def get_person_embedding(self, row_data: Dict[str, Any], feature_type: str) -> Optional[List[float]]:
    ...
    def set_person_embedding(self, row_data: Dict[str, Any], feature_type: str, embedding: List[float]) -> bool:
    ...
    def get_cached_people_embeddings(self, df: pd.DataFrame, feature_type: str) -> Dict[int, List[float]]:
    ...
    def cache_people_embeddings(self, df: pd.DataFrame, feature_type: str, embeddings: Dict[int, List[float]]) -> None:
    ...
    def get_dataset_embedding_cache_status(self, df: pd.DataFrame, feature_type: str) -> Dict[str, Any]:
        ...
        logger.info(f"Found {len(cached_embeddings)} cached {feature_type} embeddings, {len(uncached_indices)} need computing")
    ...
    def cache_dataset_embeddings(self, df: pd.DataFrame, feature_type: str, embeddings: Dict[int, List[float]]) -> None:
    ...
    def _extract_row_data(self, row: pd.Series, feature_type: str) -> Dict[str, Any]:
        ...
        if feature_type == "role":
            return {"role_spec": str(row.get("Professional Identity - Role Specification", ""))}
        elif feature_type == "experience":
            return {"experience": str(row.get("Professional Identity - Experience Level", ""))}
        elif feature_type == "persona":
            return {"personas": str(row.get("All Persona Titles", ""))}
        elif feature_type == "industry":
            return {"industry": str(row.get("Company Identity - Industry Classification", ""))}
        elif feature_type == "market":
            return {"market": str(row.get("Company Market - Market Traction", ""))}
        elif feature_type == "offering":
            return {"offering": str(row.get("Company Offering - Value Proposition", ""))}
        else:
    ...
    def get_profile_complementarity(self, target_profile: str, comparison_profiles: List[str], category: str) -> Optional[Dict[str, float]]:
        """Get cached complementarity scores for a target profile against specific comparison profiles"""
        cache_key = self._get_complementarity_cache_key(target_profile, comparison_profiles, category)
    ...
    def set_profile_complementarity(self, target_profile: str, comparison_profiles: List[str], category: str, scores: Dict[str, float]) -> bool:
    ...
    def get_dataset_complementarity_cache_status(self, target_profiles: List[str], comparison_profiles: List[str], category: str) -> Dict[str, Any]:
        """Check which target profiles have cached complementarity vs which need computing"""
        cached_results = {}
        uncached_targets = []
        
        for target in target_profiles:
            cached_scores = self.get_profile_complementarity(target, comparison_profiles, category)
            if cached_scores:
                cached_results[target] = cached_scores
            else:
                uncached_targets.append(target)
        
        logger.info(f"Found {len(cached_results)} cached {category} complementarity profiles, {len(uncached_targets)} need computing")
    ...
    def cache_dataset_complementarity_results(self, results: Dict[str, Dict[str, float]], comparison_profiles: List[str], category: str) -> None:
        """Cache complementarity results for a dataset"""
        cached_count = 0
        
        for target_profile, scores in results.items():
            if self.set_profile_complementarity(target_profile, comparison_profiles, category, scores):
                cached_count += 1
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        try:
            person_embedding_keys = len(list(self.cache.redis_client.scan_iter(match="person_embedding:*")))
            profile_comp_keys = len(list(self.cache.redis_client.scan_iter(match="profile_comp:*")))
